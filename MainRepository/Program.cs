...
namespace Test2
{
    class LetterFrequency
    {
        public static void Main(string[] args)
        {
            ...
            foreach (KeyValuePair<char, int> ele1 in result1)
            {
                Console.WriteLine("{0} is {1}", ele1.Key, ele1.Value);
            }
            Console.WriteLine();
            List<char> keys = new List<char>();
            foreach (KeyValuePair<char, int> ele2 in result1)
            {
                keys.Add(ele2.Key);
            }
